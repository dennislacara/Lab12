...
class Model:
    ...
    def cammino_minimo(self, soglia: int):
        ...
        for key_partenza in percorsi_minimi:
            for key_arrivo in percorsi_minimi[key_partenza]:
                # due nodi possono essere collegati attraverso più percorsi minimi con lo stesso peso
                # siccome non è specificato come gestire la situazione, itero su tutti i percorsi minimi di una coppia anche se verrà selezionato solo il primo
                # percorso utile; questo sarà dovuto alla comparazione booleana alla riga 95.
    ...
    def cammino_minimo_ricorsione(self, soglia):
        ...
        for nodo in g.nodes():
            self._ricorsione(g, [nodo], 0)

        return self.cammino_minimo

    def _ricorsione(self, grafo, parziale, peso_parziale):
        # pruning: se il peso parziale già supera il minimo trovato, interrompo
        if peso_parziale >= self.peso_minimo:
            return

        #condizione necessaria
        if len(parziale) >= 3:
            # se e solo se il peso del percorso parziale è inferiore all'attuale peso minimo
            if peso_parziale < self.peso_minimo:
                self.cammino_minimo = parziale.copy()
        ...
        for vicino in grafo.neighbors(nodo):
            if vicino in parziale:
                continue
            peso_arco = grafo[nodo][vicino]['peso']
            parziale.append(vicino)
            self._ricorsione(grafo, parziale, peso_parziale + peso_arco)
            #backtracking
            parziale.pop()
